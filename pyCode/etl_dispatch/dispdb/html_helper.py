...
class PageInfo:
    ...
    def all_page_count(self):
        ...
        print('all_page_count', all_page_count)
...
def pager(page,count):
    ...
    """
    page_html = []
    first_page = "<a href='/dbinfo/%d'>首页</a>" % (1)
    page_html.append(first_page)
    if page <= 1:
        prv_html = "<a href='#'>上一页</a>"
    else:
        prv_html = "<a href='/dbinfo/%d'>上一页</a>" % (page - 1)
    ...
    for i in range(begin, end):
        if page == i + 1:
            a_html = "<a style='color:red' href='/dbinfo/%d'>%d</a>" % (i + 1, i + 1)
        else:
            a_html = "<a href='/dbinfo/%d'>%d</a>" % (i + 1, i + 1)
        page_html.append(a_html)
    if page >= count:
        next_html = "<a href='#'>下一页</a>"
    else:
        next_html = "<a href='/dbinfo/%d'>下一页</a>" % (page + 1)
    page_html.append(next_html)
    if count==0:
        end_page = "<a href='#'>尾页</a>"
    else:
        end_page = "<a href='/dbinfo/%d'>尾页</a>" % (count)
