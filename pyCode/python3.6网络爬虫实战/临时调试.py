...
class Main {
    ...
    public static void main(String[] args) throws IOException{
        ...
        for(i=1;i<=Count;i++)
        {
            filePath="C:\\Users\\chenlun\\IdeaProjects\\HelloWorld\\input\\Shakespeare"+i+".txt";
            readTxtFile(filePath,root);
        }
        result=getWordCount(root);
        Iterator<word_count> itr = result.iterator();
        fw=new FileWriter("C:\\Users\\chenlun\\IdeaProjects\\HelloWorld\\output\\5.txt");
        str=new String();
...
for i in range(1,10001):
    f=open('D:\\workspace\\javaCode\\hadoop\\input\\Shakespeare'+str(i)+'.txt','w',encoding='utf-8')
    f.write(string)
    f.close()