            ...
            <div class="portlet">
                <div class="portlet-body">
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                        <#list list as reqList>
                        <tr>
                            <td>
                            ${reqList.enterpriseName?if_exists}
                            </td>
                            <td>
                            ${reqList.reportTitle?if_exists}
                            </td>
                            <td>
                            ${reqList.remark?if_exists}
                            </td>
                            <td>
                            <#--<button class="btn" href="/financeinput/tableEdit?id=${fiExcleSheet.id?if_exists}">编辑</button>-->
                                <a href="/financeinput/downloadFile?type=1&fileName=${reqList.fileName}">下载</a>
                            </td>
                            <td>
                            <#--<button class="btn" href="/financeinput/tableEdit?id=${fiExcleSheet.id?if_exists}">编辑</button>-->
                                <a href="/financeinput/dynamicReportDataDetail?reqId=${reqList.id?c}">编辑</a>

                                <a href="/financeinput/dynamicReportDataWuxiao?reqId=${reqList.id?c}">无效报表</a>
                            </td>
